...
describe('Recipes API', () => {
  describe('Average calories GET request', () => {
    ...
    test('It returns average calories for food type(Edamam API)', () => {
      mockAxios.get.mockImplementationOnce(() =>
        Promise.resolve(recipeResponse)
      );

      return request(app).get('/api/v1/recipes/avg_calories?food_type=chicken')
        .then(response => {
    ...
    test('It returns average calories for food type(database)', async () => {
      await recipe.create({
          name: "Chicken Gravy",
          foodType: "chicken",
      ...
      return request(app).get('/api/v1/recipes/avg_calories?food_type=chicken')
        .then(response => {
          expect(response.status).toBe(200)
          expect(response.body.averageCalories).toBe(2430.16)
        });
    });
  });
});
