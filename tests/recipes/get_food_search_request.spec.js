...
describe('Recipes API', () => {
  describe('Food search GET request', () => {
    ...
    test('It returns a list of recipes by food type(from Edamam)', async () => {
      mockAxios.get.mockImplementationOnce(() =>
        Promise.resolve(recipeResponse)
      );

      return await request(app).get('/api/v1/recipes/food_search?food_type=chicken')
        .then(async response => {
    ...
    test('It returns a list of recipes by food type(database)', async () => {
      await recipe.create({
          name: "Chicken Gravy",
          foodType: "chicken",
      ...
      return request(app).get('/api/v1/recipes/food_search?food_type=chicken')
        .then(response => {
          expect(response.status).toBe(200)
