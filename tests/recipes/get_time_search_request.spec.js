...
describe('Recipes API', () => {
  describe('Time search GET request', () => {
    ...
    test('It returns a list of recipes by max total cook/prep time(Edamam API)', () => {
      mockAxios.get.mockImplementationOnce(() =>
        Promise.resolve(timeResponse)
      );

      return request(app).get('/api/v1/recipes/time_search?food_type=tongue&max=20')
        .then(response => {
    ...
    test('It returns a list of recipes by max total cook/prep time(database)', async () => {
      await recipe.create({
          name: "Chicken Gravy",
          foodType: "chicken",
      ...
      return request(app).get('/api/v1/recipes/time_search?food_type=chicken&max=20')
        .then(response => {
          expect(response.status).toBe(200)
